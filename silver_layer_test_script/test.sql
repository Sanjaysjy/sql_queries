...
WITH LoanApplicationDetails AS (
    ...
    left JOIN dmihfclos.mstLoanPurpose lp ON la.loanPurposeID = lp.loanPurposeID
            AND lp.isActive = 1

    left JOIN dmihfclos.mstProduct p ON la.loanProductID = p.productID
            AND p.isActive = 1

    left JOIN dmihfclos.tblLoanApplicationDisbursalDetail ds ON la.loanApplicationID = ds.loanApplicationID
            AND  ds.isActive = 1

    left JOIN dmihfclos.tblLoanApplicationAdditionalDetail lad ON la.loanApplicationID = lad.loanApplicationID
            AND lad.isActive = 1

    left JOIN dmihfclos.tblLoanApplicationStatusHistory lsh ON la.loanApplicationID = lsh.loanApplicationID
            AND lsh.isActive = 1

    left JOIN dmihfclos.tblTypeDetail ts ON la.loanSchemeTypeDetailID = ts.typeDetailID
            AND ts.isActive = 1

    left JOIN dmihfclos.tblTypeDetail lt ON la.interestTypeTypeDetailID = lt.typeDetailID
        AND lt.isActive = 1
...
LEFT JOIN audit_details ad
    ON ls.loan_application_id = ad.loan_application_id
-- )    select count(loan_application_id) from final

    -- select loan_application_id, count(*) from final group by loan_application_id  having  count(loan_application_id) >1